...
export default function ProviderForm({
  ...
  const [
    updateProvider,
    {
      ...
      reset: resetUpdateProvider,
    },
  ] = useUpdateProviderMutation()
  const [
    configureProvider,
    {
      data: configureProviderData,
      error: configureProviderError,
      loading: configureProviderLoading,
      reset: resetConfigureProvider,
    },
  ] = useConfigrueProviderMutation()
  const [
    loadProvider,
  ...
  const [
    unloadProvider,
    {
      data: unloadProviderData,
      error: unloadProviderError,
      loading: unloadProviderLoading,
      reset: resetUnloadProvider,
    },
  ] = useUnloadProviderMutation()
  ...
  useEffect(() => {
    if (getProviderError)
      enqueueSnackbar({
        message: `Failed to get provider: ${getProviderError.message}`,
        variant: 'error',
      })
    if (createProviderError)
      enqueueSnackbar({
        message: `Failed to create provider: ${createProviderError.message}`,
        variant: 'error',
      })
    if (updateProviderError)
      enqueueSnackbar({
        message: `Failed to update provider: ${updateProviderError.message}`,
        variant: 'error',
      })
    if (configureProviderError)
      enqueueSnackbar({
        message: `Failed to reconfigure provider: ${configureProviderError.message}`,
        variant: 'error',
      })
    if (loadProviderError)
      enqueueSnackbar({
        message: `Failed to load provider: ${loadProviderError.message}`,
        variant: 'error',
      })
    if (unloadProviderError)
      enqueueSnackbar({
        message: `Failed to unload provider: ${unloadProviderError.message}`,
        variant: 'error',
      })
  }, [
    getProviderError,
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      if (provider.providerGitUrl) {
        const repoPath = provider.providerGitUrl
    ...
    <Container sx={{ py: 3 }}>
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <TextField
            ...
            onChange={(e) =>
              setProvider((prev) => ({ ...prev, displayName: e.target.value }))
            }
          ></TextField>
          {getProviderData && (
            <>
              <Circle
                sx={{ ml: 1, height: '1rem' }}
                color={getProviderData.provider.isLoaded ? 'success' : 'error'}
              />
              <Typography
                variant="subtitle1"
                color={
                  getProviderData.provider.isLoaded
                    ? 'success.main'
                    : 'error.main'
                }
              >
          ...
          {getProviderData && (
            <ButtonGroup sx={{ mr: 1 }}>
              <LoadingButton
                ...
                startIcon={<Cached />}
                disabled={
                  !getProviderData.provider.isLoaded || updateProviderLoading
                }
                loading={configureProviderLoading}
                onClick={() => {
                  if (getProviderData.provider.isLoaded)
                    configureProvider({
                      variables: { id: getProviderData.provider.id },
                    }).catch(console.error)
                }}
              >
                Reconfigure
              ...
              <LoadingButton
                ...
                startIcon={
                  getProviderData.provider.isLoaded ? <PowerOff /> : <Power />
                }
      ...
      <Grid container spacing={2}>
        ...
        <Grid item md={6}>
          <Autocomplete
            ...
            sx={{ width: '100%' }}
            placeholder="v1.0.0"
            options={gitTags.map((option) =>
              option.ref.replace('refs/tags/', '')
            )}
            renderInput={(params) => <TextField {...params} label="Version" />}
            value={provider.providerVersion}
            onChange={(_, value) =>
              setProvider((prev) => ({ ...prev, providerVersion: value || '' }))
            }
      ...
      >
        <MonacoEditor
          ...
          onChange={(value, _) =>
            setProvider((prev) => ({ ...prev, configBytes: value ?? '' }))
          }
        ></MonacoEditor>
      </Box>
    </Container>
