...
export default function Providers() {
  ...
  useEffect(() => {
    if (listProvidersError)
      enqueueSnackbar({
        message: `Failed to list providers: ${listProvidersError.message}`,
        variant: 'error',
      })
    ...
    if (unloadProviderError)
      enqueueSnackbar({
        message: `Failed to unload providers: ${unloadProviderError.message}`,
        variant: 'error',
      })
  ...
  useEffect(() => {
    if (loadProviderData) {
      enqueueSnackbar({
        message: `Loaded provider "${loadProviderData.loadProvider.displayName}"`,
        variant: 'success',
      })
      resetLoadProvider()
    ...
    if (unloadProviderData) {
      enqueueSnackbar({
        message: `Unloaded provider "${unloadProviderData.unloadProvider.displayName}"`,
        variant: 'success',
      })
      resetUnloadProvider()
      refetchListProviders()
              ...
              <CardContent>
                <Grid container spacing={1}>
                  ...
                  >
                    ...
                    <Circle
                      color={provider.isLoaded ? 'success' : 'error'}
                      sx={{ ml: 1, height: '1rem' }}
                    />
                    <Typography
                      variant="subtitle1"
                      color={provider.isLoaded ? 'success.main' : 'error.main'}
                    >
                  ...
                  <Grid item xs={12}>
                    <Typography variant="body1">
                      Git Source:{' '}
                      <TypographyCode>{provider.providerGitUrl}</TypographyCode>
                    </Typography>
                    <Typography variant="body1">
                      Version:{' '}
                      <TypographyCode>
      ...
      >
        <MenuItem
          onClick={() => {
            ...
            if (moreMenuProvider.isLoaded)
              unloadProvider({ variables: { id: moreMenuProvider.id } }).catch(
                console.error
              )
            else
              loadProvider({ variables: { id: moreMenuProvider.id } }).catch(
                console.error
              )
          }}
          disabled={loadProviderLoading || unloadProviderLoading}
        >
          {loadProviderLoading || unloadProviderLoading ? (
            <CircularProgress size="1rem" />
          ) : moreMenuProvider?.isLoaded ? (
            'Unload'
          ) : (
            'Load'
          )}
        ...
        <MenuItem
          onClick={() => navigate(`/providers/edit/${moreMenuProvider?.id}`)}
        >
          Edit
        </MenuItem>
      </Menu>
