...
func (pq *PermissionQuery) QueryPermissionPolicies() *PermissionPolicyQuery {
	query := (&PermissionPolicyClient{config: pq.config}).Query()
...
func (pq *PermissionQuery) Clone() *PermissionQuery {
	...
	return &PermissionQuery{
		...
		withPermissionPolicies: pq.withPermissionPolicies.Clone(),
...
func (pq *PermissionQuery) WithPermissionPolicies(opts ...func(*PermissionPolicyQuery)) *PermissionQuery {
	...
	pq.withPermissionPolicies = query
	return pq
}
...
func (pq *PermissionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Permission, error) {
	...
	if query := pq.withPermissionPolicies; query != nil {
		if err := pq.loadPermissionPolicies(ctx, query, nodes,
			func(n *Permission) { n.Edges.PermissionPolicies = []*PermissionPolicy{} },
			func(n *Permission, e *PermissionPolicy) {
...
func (pq *PermissionQuery) loadPermissionPolicies(ctx context.Context, query *PermissionPolicyQuery, nodes []*Permission, init func(*Permission), assign func(*Permission, *PermissionPolicy)) error {
	...
	query.Where(predicate.PermissionPolicy(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(permission.PermissionPoliciesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.permission_policy_permission
		if fk == nil {
			return fmt.Errorf(`foreign-key "permission_policy_permission" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "permission_policy_permission" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
