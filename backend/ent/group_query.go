...
func (gq *GroupQuery) QueryProjects() *ProjectQuery {
	query := (&ProjectClient{config: gq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := gq.withProjects; query != nil {
		if err := gq.loadProjects(ctx, query, nodes,
			func(n *Group) { n.Edges.Projects = []*Project{} },
			func(n *Group, e *Project) { n.Edges.Projects = append(n.Edges.Projects, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GroupQuery) loadProjects(ctx context.Context, query *ProjectQuery, nodes []*Group, init func(*Group), assign func(*Group, *Project)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*Group)
	nids := make(map[uuid.UUID]map[*Group]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(group.ProjectsTable)
		s.Join(joinT).On(s.C(project.FieldID), joinT.C(group.ProjectsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(group.ProjectsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.ProjectsPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
