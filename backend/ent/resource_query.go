...
type ResourceQuery struct {
	...
	withDependsOn  *ResourceQuery
	withRequiredBy *ResourceQuery
	withFKs        bool
...
func (rq *ResourceQuery) QueryBlueprint() *BlueprintQuery {
	query := (&BlueprintClient{config: rq.config}).Query()
...
func (rq *ResourceQuery) Clone() *ResourceQuery {
	...
	return &ResourceQuery{
		...
		withBlueprint:  rq.withBlueprint.Clone(),
		withDependsOn:  rq.withDependsOn.Clone(),
		withRequiredBy: rq.withRequiredBy.Clone(),
...
func (rq *ResourceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Resource, error) {
	var (
		...
		_spec       = rq.querySpec()
		loadedTypes = [3]bool{
			rq.withBlueprint != nil,
			rq.withDependsOn != nil,
			rq.withRequiredBy != nil,
		}
	)
	if rq.withBlueprint != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := rq.withBlueprint; query != nil {
		if err := rq.loadBlueprint(ctx, query, nodes, nil,
			func(n *Resource, e *Blueprint) { n.Edges.Blueprint = e }); err != nil {
			return nil, err
		}
	}
	if query := rq.withDependsOn; query != nil {
		if err := rq.loadDependsOn(ctx, query, nodes,
			func(n *Resource) { n.Edges.DependsOn = []*Resource{} },
			func(n *Resource, e *Resource) { n.Edges.DependsOn = append(n.Edges.DependsOn, e) }); err != nil {
			return nil, err
		}
	}
	if query := rq.withRequiredBy; query != nil {
		if err := rq.loadRequiredBy(ctx, query, nodes,
			func(n *Resource) { n.Edges.RequiredBy = []*Resource{} },
			func(n *Resource, e *Resource) { n.Edges.RequiredBy = append(n.Edges.RequiredBy, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *ResourceQuery) loadBlueprint(ctx context.Context, query *BlueprintQuery, nodes []*Resource, init func(*Resource), assign func(*Resource, *Blueprint)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Resource)
	for i := range nodes {
		if nodes[i].resource_blueprint == nil {
			continue
		}
		fk := *nodes[i].resource_blueprint
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "resource_blueprint" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (rq *ResourceQuery) loadDependsOn(ctx context.Context, query *ResourceQuery, nodes []*Resource, init func(*Resource), assign func(*Resource, *Resource)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(resource.DependsOnTable)
		s.Join(joinT).On(s.C(resource.FieldID), joinT.C(resource.DependsOnPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(resource.DependsOnPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(resource.DependsOnPrimaryKey[1]))
...
func (rq *ResourceQuery) loadRequiredBy(ctx context.Context, query *ResourceQuery, nodes []*Resource, init func(*Resource), assign func(*Resource, *Resource)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uuid.UUID]*Resource)
	nids := make(map[uuid.UUID]map[*Resource]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(resource.RequiredByTable)
		s.Join(joinT).On(s.C(resource.FieldID), joinT.C(resource.RequiredByPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(resource.RequiredByPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(resource.RequiredByPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "required_by" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
